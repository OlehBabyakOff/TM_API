...
export const createEventService = async (userId, title, description, startDate, endDate) => {
    if (!userId && !title && !description) throw new Error('Empty values are not allowed!');
    ...
    const startCheck = await EventSchema.find({user:userId, $and: [{startDate: {$lte: startDate}}, {endDate: {$gte: startDate}}]});
    const endCheck = await EventSchema.find({user:userId, $and: [{startDate: {$lte: endDate}}, {endDate: {$gte: endDate}}]});

    if (startCheck.length || endCheck.length) throw new Error('You can’t create event for this time');
